import {
  sidebar, menu, constantsArray,
  correctAnswerSound, wrongAnswerSound, successSound, failureSound, tableHeaders
} from './constants.js';

import {
  mainContainer, headerContainer, navBurger, overlayDiv,
  trainCaption, playCaption,
  navSidebar, starsContainer, startPlayBtn,
  resultDiv, loseCaption, createMainLayout
} from './mainLayout.js';
  ...
  checkAnswer(text) {
    ...
    if (this.randomArr[0] === indexOfClickedCard) {
      ...
      if (this.randomArr.length > 0) {
        ...
        document.querySelectorAll('.item-div')[indexOfClickedCard].classList.add('answered');
  ...
  createStatsPage() {
    ...
    for (let rowCurr = 0; rowCurr < rowsNumber; rowCurr++) {
      ...
      for (let cellCurr = 0; cellCurr < cellsInRowNumber; cellCurr++) {
        ...
        switch (cellCurr) {
          case 0: cell.textContent = object.word; break;
          case 1: cell.textContent = sidebar[categoryIndex]; break;
          case 2: cell.textContent = object.translation; break;
          case 3: cell.textContent = trained; break;
          case 4: cell.textContent = object.correct; break;
          case 5: cell.textContent = object.mistakes; break;
          case 6: {
            if (trained === 0) {
              cell.textContent = '0%'; break;
            } else {
      ...
      for (let i = 0; i < divCaptionsEng.length; i++) {
        ...
        if (!sidebar.includes(element.textContent)) {
          if (playCaption.classList.contains('hidden')) {
            ...
            element.closest('.front').querySelector('.item-image').classList.remove('wide');
          } else {
            this.clearStars();
            captionFront.classList.add('hidden');
            element.closest('.front').querySelector('.item-image').classList.add('wide');
          }
        }
      }
    });
    ...
    for (let i = 0; i < divcaptionsBY.length; i++) {
      ...
      itemDiv.addEventListener('mouseleave', () => {
        if (document.querySelector('.active-rotate')) {
          document.querySelector('.active-rotate').classList.remove('active-rotate');
        }
